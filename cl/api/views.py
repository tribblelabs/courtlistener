...
from cl.search.api_serializers import CitationLookupSerializer
...
@api_view(['POST'])
def citation_lookup_view(request):
    serializer = CitationLookupSerializer(data=request.data)
    if serializer.is_valid():
        citation_str = serializer.validated_data['citation']
        citation_objs = get_citations(citation_str)
        matched_resources = do_resolve_citations(citation_objs, None)

        cluster_ids = [
            resource.cluster_id for resource, _ in matched_resources.items()
            if isinstance(resource, Opinion)
        ]

        if len(cluster_ids) == 1:
            return JsonResponse({'cluster_id': cluster_ids[0]}, status=status.HTTP_200_OK)
        elif len(cluster_ids) > 1:
            return JsonResponse({'cluster_ids': cluster_ids}, status=status.HTTP_300_MULTIPLE_CHOICES)
        else:
            return JsonResponse({'error': 'Citation not found'}, status=status.HTTP_404_NOT_FOUND)
    else:
